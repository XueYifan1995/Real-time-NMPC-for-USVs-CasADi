function Draw_MPC_PSship_tracking_dynamicObstacles (t,xx,xx1,u_cl,xs,N,rob_diam,obs_diam,USV_rf)
...
tic
for k = 1:size(xx,2)
    h_t = 2.44; w_t=1.22; % triangle parameters
...
%     hold on;
    ...
    x1_tri = [ x1+h_t*cos(th1), x1+(w_t/2)*cos((pi/2)-th1), x1-(w_t/2)*cos((pi/2)-th1)];%,x1+(h_t/3)*cos(th1)];
    y1_tri = [ y1+h_t*sin(th1), y1-(w_t/2)*sin((pi/2)-th1), y1+(w_t/2)*sin((pi/2)-th1)];%,y1+(h_t/3)*sin(th1)];
    plot(x_r_1,y_r_1,'-r','linewidth',line_width);axis([-10 10 0 60]);hold on % plot exhibited trajectory    
    if k < size(xx,2) % plot prediction
        plot(xx1(1:N,4,k),xx1(1:N,5,k),'r--*');axis([-10 10 0 60]);
        for j = 2:N+1
            plot(xx1(j,4,k)+xp,xx1(j,5,k)+yp,'--r');axis([-10 10 0 60]); % plot robot circle
        end
    end
    ...
    plot(USV_rf(4,:),USV_rf(5,:),'k--','LineWidth',1.5);
...
figure
subplot(511)
plot(t',U_pre,'linewidth',1.5);xlabel('time (seconds)');ylabel('u (m/s)');grid on
subplot(512)
plot(t',V_pre,'linewidth',1.5);xlabel('time (seconds)');ylabel('v (m/s)');grid on
subplot(513)
plot(t',R_pre,'linewidth',1.5);xlabel('time (seconds)');ylabel('r (rad/s)');grid on
subplot(514)
stairs(t,u_cl(:,1),'r','linewidth',1.5); xlabel('time (seconds)');ylabel('T_port (N)');grid on
subplot(515)
stairs(t,u_cl(:,2),'r','linewidth',1.5); xlabel('time (seconds)');ylabel('T_stbd (N)');grid on

function Draw_MPC_PSship_tracking8 (t,xx,xx1,u_cl,xs,N,rob_diam,USV_rf)
...
%     hold on;
    ...
    x_r_1 = [x_r_1 x1];
    y_r_1 = [y_r_1 y1];
    x1_tri = [ x1+h_t/2*cos(th1), x1+(w_t/2)*cos((pi/2)-th1), x1-(w_t/2)*cos((pi/2)-th1)];%,x1+(h_t/3)*cos(th1)];
    y1_tri = [ y1+h_t/2*sin(th1), y1-(w_t/2)*sin((pi/2)-th1), y1+(w_t/2)*sin((pi/2)-th1)];%,y1+(h_t/3)*sin(th1)];
...
figure
subplot(511)
plot(t',U_pre,'linewidth',1.5);xlabel('time (seconds)');ylabel('u (m/s)'); axis([0 65 0 2]);grid on
subplot(512)
plot(t',V_pre,'linewidth',1.5);xlabel('time (seconds)');ylabel('v (m/s)'); axis([0 65 -0.21 0.21]);grid on
subplot(513)
plot(t',R_pre,'linewidth',1.5);xlabel('time (seconds)');ylabel('r (rad/s)');axis([0 65 -1 4]);grid on
subplot(514)
stairs(t,u_cl(:,1),'r','linewidth',1.5); xlabel('time (seconds)');ylabel('T_port (N)');axis([0 65 -100 410]);grid on
subplot(515)
stairs(t,u_cl(:,2),'r','linewidth',1.5); xlabel('time (seconds)');ylabel('T_stbd (N)');axis([0 65 -100 410]);grid on

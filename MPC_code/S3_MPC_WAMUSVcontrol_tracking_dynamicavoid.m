...
obs = SX.sym('obs');
states = [u;v;r;x;y;psi;obs]; n_states = length(states);
...
Q = zeros(7,7); Q(1,1)=1; Q(2,2)=0.1;Q(3,3)=0.1; Q(4,4) = 100;Q(5,5) = 100;Q(6,6) = 0.1; % weighing matrices (states)
R = zeros(2,2); R(1,1) = 0.03;  R(2,2) = 0.001;% weighing matrices (controls)
% compute objective
for k=1:N
    st = X(:,k);  con = U(:,k);
    obj = obj+(st-P(8:14))'*Q*(st-P(8:14)) + con'*R*con; % calculate obj
end
...
for k = 1:N+1   % box constraints due to the map margins
    g = [g ; -sqrt((X(4,k)-obs_x)^2+(X(5,k)-X(7,k))^2) + (4.88+0.5)];
end
...
%-------------------------------------------
t0 = 0;
x0 = [0;0;0; 0 ; 0 ; 90*pi/180; 55];    % initial condition.

load ref_usv_line2 ref_usv_line2
ref = ref_usv_line2(1:7,1:1:end);       % Reference trajectory
...
while(mpciter < sim_tim / T)
    ...
    xx1(:,1:7,mpciter+1)= full(ff_value)';
...
% X_pre = xx(4,:);Y_pre = xx(5,:); YY_pre=xx(7,:);
% plot(X_pre,Y_pre);hold on
% plot(zeros(1,301),YY_pre);axis([-5 5 0 60])

obs_diam = 4;
% Draw_MPC_PSship_tracking_dynamicObstacles (t,xx,xx1,u_cl,xs,N,rob_diam,obs_diam,ref)